...
/* change */
#define ICSK_CA_PRIV_SIZE (16*sizeof(s64))
#define round 5 /* length of monitor round */
#define distance 250 /* 0.5 is actually used -> 0.5*0.5*PCC_SCALE */
/* change */
...
struct pcc_data
{
	...
	/* change */
	s64 curr_grad; /* current gradient */
	s64 last_grad; /* last MI gradient */
	s64 curr_rate; /* current rate */
	int monitor_flag;
	int monitor_round;
	/* s64 last_rate; */
	/* change */
...
static void start_interval(struct sock *sk, struct pcc_data *pcc)
{
	...
	/* change */
	if (pcc->monitor_flag == 1) {
		tp->snd_cwnd++;
		pcc->recive_index++;
		pcc->monitor_round++;
		/* Exit the monitoring phase when packet loss rate is more than 2% */
		if ((interval->lost + interval->delivered) / interval->lost < 50) {
			pcc->monitor_flag = 0;
			pcc->monitor_round = 0;
		}
	}
	else if (pcc->curr_grad * pcc->last_grad < 0) {
		pcc->monitor_flag = 1;
		rate = pcc->curr_rate;
		...
		pcc_set_cwnd(sk);
		DBG_PRINT(KERN_INFO "%hhu: entering network monitoring phase with rate %u (%u)\n", pcc->id, rate, TO_MPBPS(rate));
		pcc->recive_index = 0;
	}
	else {
		pcc->monitor_flag = 0;
...
static void pcc_calc_utility_vivace(struct pcc_data *pcc, struct pcc_interval *interval, struct sock *sk)
{
	...
	/* change */
	fixedpt reward_sf = 0;
	other_rate = pcc->others_rate;

	reward_sf = fixedpt_fromint(other_rate);
	reward_sf = fixedpt_pow(reward_sf, fixedpt_rconst(0.9));
...
/* change */
static void pcc_decide_monitor(struct pcc_data *pcc, struct sock *sk)
{
	struct tcp_sock *tp = tcp_sk(sk);
	struct pcc_interval *interval;
	tp->snd_cwnd++;

	s64 former_loss_ratio, present_loss_ratio;
	s64 former_end_rtt, present_end_rtt;
	s64 max_loss_ratio = 0;
	s64 min_loss_ratio = 0;
	s64 max_end_rtt = 0;
	s64 min_end_rtt = 0;
	s64 center_L1, center_R1;
	s64 center_L2, center_R2;
	u32 d;
	int i;
/*	for (i = 0; i < round; i++)
	{
		...
		former_utility += interval->utility;
	}
	for (i = round; i < round*2; i++)
	{
		interval = &pcc->intervals[i];
		pcc_calc_utility_vivace(pcc, interval, sk);
		present_utility += interval->utility;
	}

	if (present_utility/former_utility > distance || former_utility/present_utility > distance) {	// network state changes measurably
		pcc->monitor_flag = 0;
		pcc->curr_grad = 0;
		pcc->last_grad = 0;
		DBG_PRINT(KERN_INFO "%hhu: Network state changes measurably, restart the online learning process!", pcc->id);
	} */
	for (i = 0; i < round; i++)
	{
		interval = &pcc->intervals[i];
		if (interval->loss_ratio > max_loss_ratio) {
			max_loss_ratio = interval->loss_ratio;
		}
		if (interval->loss_ratio < min_loss_ratio) {
			min_loss_ratio = interval->loss_ratio;
		}
		if (interval->end_rtt > max_end_rtt) {
			max_end_rtt = interval->end_rtt;
		}
		if (interval->end_rtt < min_end_rtt) {
			min_end_rtt = interval->end_rtt;
		}
		former_loss_ratio += interval->loss_ratio;
		former_end_rtt += interval->end_rtt;
	}
	for (i = round; i < round*2; i++)
	{
		interval = &pcc->intervals[i];
		...
		present_loss_ratio += interval->loss_ratio;
		present_end_rtt += interval->end_rtt;
	}
	
	center_L1 = (former_loss_ratio*PCC_SCALE / (round*(max_loss_ratio-min_loss_ratio))) - (min_loss_ratio*PCC_SCALE/(max_loss_ratio-min_loss_ratio));
	center_R1 = (former_end_rtt*PCC_SCALE / (round*(max_end_rtt-min_end_rtt))) - (min_end_rtt*PCC_SCALE/(max_end_rtt-min_end_rtt));
	center_L2 = (present_loss_ratio*PCC_SCALE / (round*(max_loss_ratio-min_loss_ratio))) - (min_loss_ratio*PCC_SCALE/(max_loss_ratio-min_loss_ratio));
	center_R2 = (present_end_rtt*PCC_SCALE / (round*(max_end_rtt-min_end_rtt))) - (min_end_rtt*PCC_SCALE/(max_end_rtt-min_end_rtt));
	d = (center_L2-center_L1)*(center_L2-center_L1) + (center_R2-center_R1)*(center_R2-center_R1);

	if (d > PCC_SCALE*distance) {	// network state changes measurably
		pcc->monitor_flag = 0;
		pcc->curr_grad = 0;
		pcc->last_grad = 0;
		DBG_PRINT(KERN_INFO "%hhu: Network state changes measurably, restart the online learning process!", pcc->id);
...
static void pcc_decide_monitor_moving(struct sock *sk, struct pcc_data *pcc)
{
...
static u32 pcc_decide_rate_moving(struct sock *sk, struct pcc_data *pcc)
{
	...
	/* change */
	pcc->last_grad = pcc->curr_grad;
	pcc->curr_grad = grad;
	pcc->curr_rate = pcc->rate;
	/* change */
...
static void pcc_process(struct sock *sk)
{
	...
	if (recive_interval_ended(interval, tsk, pcc))
	{
		...
		/* change */
		else if (pcc->monitor_flag == 1) {
			if (pcc->moving)
				pcc_decide_monitor_moving(sk, pcc);
			else if (pcc->monitor_round >= round*2) {
				pcc_decide_monitor(pcc, sk);
				pcc->monitor_round = 0;
			}
		}
		else {
...
MODULE_AUTHOR("Zhuang Rui");
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("MPTCP ONLINE LEARNING");
MODULE_VERSION("0.1");
